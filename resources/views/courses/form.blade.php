<div class=" mb-4  pl-2 form-group" >
        ...
        <input type="text"  value="{{old('name') ?? $course->name }} " name="name" id="name" class="mb-3 ml-1 w-75 h-75% pl-4 rounded-pill form-control">
        <div class="text-right  w-75">{{$errors->first('name')}}</div>

    </div>
    <div class=" mb-4  pl-2 form-group">
    <label for="email">Email:</label>
    <input type="text" name="email" id="email" class="mb-3 ml-1  w-75 h-75% rounded-pill form-control" value="{{old('email') ?? $course->email }}">
    <div class="text-right  w-75">{{$errors->first('email')}}</div>
    </div>
    <div class="form-group mb-4 ml-3 text -center">
